...
> **最新 dev 部署（2026-09-25）：** 已更新為 `linewebhook-00004-teb`（ACTIVE、100% 流量），新增 👆／🚧 回覆與 Firestore 失敗記錄；最近部署檢查帳本為 24,124 字元／182 次，此為檢查當時快照。詳見 [dev 回應與失敗記錄](LINEOAdev回應與失敗記錄.md)；以下 v21 數值與版本屬歷史基準，原品質 findings 與付費回歸暫停持續有效。
