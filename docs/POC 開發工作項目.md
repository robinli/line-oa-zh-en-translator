...
- 文件更新日期：2026-09-04（加入語音轉文字）
- 狀態定義：⬜ 未開始／🟡 進行中／⏸️ 阻塞／✅ 已完成
- POC 目標：群組經授權者啟用後，LINE OA 將群組中文文字翻譯為英文，並將群組語音轉為文字；一對一聊天室亦支援語音轉文字。
...
| POC-10 | 解析 Webhook event 並篩選支援的文字與語音事件 | ✅ 已完成 | 接受群組文字、群組／一對一 LINE 語音及一對一 `/我的ID`，忽略其他事件 |
...
| POC-15 | 撰寫核心邏輯自動化測試 | ✅ 已完成 | 4 個測試檔共 62 項測試通過，涵蓋簽章、啟停、文字、語音、外部 SDK request、成功與失敗流程 |
...
| POC-24 | 支援一對一語音 | ✅ 已完成 | 一對一語音不需群組啟用狀態；中文逐字稿附英文翻譯 |
| POC-25 | 更新語音功能文件與驗證 | ✅ 已完成 | README、架構、部署、工作項目、選型與日誌文件已更新，62 項測試及建置通過 |
...
部署前可完成的程式開發已完成；執行 `npm.cmd run verify` 會一次完成型別檢查、62 項測試與正式建置，Firebase 部署前也會自動執行相同檢查。

原始文字翻譯版本已完成正式環境驗收；2026-09-04 的語音擴充版本已完成程式、API、IAM 與自動化驗證，尚待重新部署及 LINE 群組／一對一人工驗收。
