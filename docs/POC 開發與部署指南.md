...
> **部分內容為舊版紀錄：** 本指南尚未完整涵蓋目前 Vertex 商務翻譯、原生提及及翻譯失敗靜默處理。開發或部署前請先閱讀 [系統規則與知識總覽](系統規則與知識總覽.md) 第 1 節及任務相關章節，再使用相關最新功能文件。

## 目前狀態

- Firebase Functions 第 2 代 TypeScript 專案，Node.js 22，區域為 asia-east1。
- 群組支援四種翻譯模式，文字翻譯與語音轉文字獨立啟停；私訊僅回覆 `/我的ID`，其餘均忽略。
- 語音先轉逐字稿，再套用文字翻譯設定；未啟用翻譯或方向不符合時，只回覆逐字稿。
- 群組僅授權者可修改設定，私訊不能查詢或修改設定。Webhook 簽章仍以原始 raw body 驗證。
...
群組的設定修改限授權者；所有群組成員可查詢狀態。私訊僅回覆 `/我的ID`。
