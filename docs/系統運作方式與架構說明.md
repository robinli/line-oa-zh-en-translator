...
本專案是一個「收到 LINE 群組訊息後才執行」的無伺服器翻譯機器人。Firebase Function 負責提供公開的 Webhook 網址、執行程式，以及管理部署、執行權限和 Secret；它不負責翻譯，真正的翻譯由 Google Cloud Translation API 完成。
...
簽章正確後，程式才會解析 JSON，接著只處理同時符合以下條件的事件：
...
Google Cloud 專案 ID 由 Firebase 執行環境提供。Function 使用專用服務帳戶取得 Translation API 權限，不需要把 Google 金鑰寫進程式碼。
...
同一個 Webhook 可能一次包含多個事件。程式使用 `Promise.allSettled()` 平行處理，最後統計：
...
Firebase Functions 在此負責五件事：

1. 提供公開 HTTPS Webhook URL。
2. 在請求抵達時執行 Node.js 程式。
3. 注入 LINE Secret。
4. 讓程式以指定服務帳戶呼叫 Google Translation API。
5. 收集執行日誌、管理擴縮與逾時。
...
因此這是一個無資料庫、無狀態的 Function。每個 Webhook request 都獨立處理。

## Secret 和一般設定的差別

[`functions/src/index.ts`](../functions/src/index.ts) 定義兩個敏感值：
...
5. [`functions/src/services.ts`](../functions/src/services.ts)：Google 翻譯和 LINE 回覆的外部 API 包裝。
