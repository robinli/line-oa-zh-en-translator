...
const fixturePath=new URL('../evaluation/synthetic-context-quantity-verifier-fresh-v11.json',import.meta.url),fixtureBytes=readFileSync(fixturePath),fixtureHash=sha(fixtureBytes);
if(fixtureHash!=='38d27bae3694abdaa0698d9191009aa267908478f534c4f7dd8535bd6e628fa4')throw Error('Frozen verifier fixture changed');
const cases=JSON.parse(fixtureBytes),reference=JSON.parse(readFileSync(new URL('../../.local/tllm-context-quantity-repairs/frozen-v11.json',import.meta.url)));
...
if(JSON.stringify(hashes)!==JSON.stringify(reference.hashes))throw Error('Candidate differs from reviewed frozen v11');
if(cases.length!==6||cases.filter(c=>c.core).length!==6)throw Error('Fixture contract changed');
...
if(process.argv.includes('--dry-run')){console.log(JSON.stringify({node:process.version,adapterVersion:TRANSLATION_LLM_ADAPTER_VERSION,fixtureHash,cases:cases.length,core:6,rounds:3,hashes}));process.exit(0);}
...
const dir=new URL('.local/tllm-context-quantity-verifier-v11/',root);mkdirSync(dir,{recursive:true});
