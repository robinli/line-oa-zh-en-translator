...
const cases = [];
const datasetHashes = {};
for (const file of ["synthetic-tllm-v10-holdout.json"]) {
...
if (cases.length !== 32 || new Set(cases.map(sample => sample.id)).size !== 32 ||
...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, datasetHashes, model: "general/translation-llm", glossaryVersion: "v8", glossaryMode: "standard-conditional-native-html-semantic-markers",
    syntheticOnly: true, expected: 32 * rounds, rounds, apiCalls: apiCalls.length,
...
for (let round = 1; round <= rounds; round++) {
  for (const sample of cases) {
    ...
    if (results.length % 10 === 0 || serviceFailure) console.log(JSON.stringify({done: results.length,
      expected: 32 * rounds, completed: results.filter(row => row.completed).length, serviceFailure}));
