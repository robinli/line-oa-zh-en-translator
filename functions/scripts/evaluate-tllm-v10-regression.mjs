...
for (const name of ["translation-llm-translator.js", "translation-llm-protection.js", "trade-policy.js", "translation-llm-subjects.js", "translation-llm-html.js"]) {
...
const cases = [];
const datasetHashes = {};
for (const file of ["synthetic-trade-cases.json", "synthetic-alternative-cases.json", "synthetic-tllm-holdout.json", "synthetic-tllm-v2-holdout.json", "synthetic-tllm-v4-holdout.json", "synthetic-tllm-v6-holdout.json", "synthetic-tllm-v8-holdout.json", "synthetic-tllm-v9-regression.json"]) {
  const bytes = await readFile(new URL("../evaluation/" + file, import.meta.url));
  datasetHashes[file] = createHash("sha256").update(bytes).digest("hex");
  const data = JSON.parse(bytes.toString("utf8"));
  cases.push(...data.filter(sample => sample.sourceLanguage !== "vi" && sample.targetLanguage !== "vi")
    .map(sample => ({...sample, set: "regression"})));
}
if (cases.length !== 212 || new Set(cases.map(sample => sample.id)).size !== 212 ||
...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, datasetHashes, model: "general/translation-llm", glossaryVersion: "v8", glossaryMode: "standard-conditional-native-html-semantic-markers",
    syntheticOnly: true, expected: 212 * rounds, rounds, apiCalls: apiCalls.length,
...
for (let round = 1; round <= rounds; round++) {
  for (const sample of cases) {
    ...
    const ranges = (sample.mentions ?? []).map(mention => {
      let start = mention.start ?? -1;
      if (mention.start === undefined) for (let i=0; i<=mention.occurrence; i++) start = sample.source.indexOf(mention.text, start + 1);
      if (!Number.isSafeInteger(start) || start < 0 || sample.source.slice(start, start + mention.text.length) !== mention.text) throw new Error("Invalid synthetic mention range.");
    ...
    const translator = new TranslationLlmTranslator({projectId, location: "us-central1",
      glossaryZhEn: parent + "/glossaries/trade-zh-en-v8", glossaryEnZh: parent + "/glossaries/trade-en-zh-v8",
    ...
    if (results.length % 10 === 0 || serviceFailure) console.log(JSON.stringify({done: results.length,
      expected: 212 * rounds, completed: results.filter(row => row.completed).length, serviceFailure}));
