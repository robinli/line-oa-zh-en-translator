...
for (const file of ["synthetic-trade-cases.json", "synthetic-alternative-cases.json", "synthetic-tllm-holdout.json", "synthetic-tllm-v2-holdout.json", "synthetic-tllm-v4-holdout.json", "synthetic-tllm-v6-holdout.json", "synthetic-tllm-v8-holdout.json", "synthetic-tllm-v9-regression.json", "synthetic-tllm-v10-holdout.json", "synthetic-tllm-v11-holdout.json"]) {
  ...
  cases.push(...data.filter(sample => sample.sourceLanguage !== "vi" && sample.targetLanguage !== "vi")
    .map(sample => ({...sample, set: file === "synthetic-tllm-v11-holdout.json" ? "holdout" : "regression"})));
}
if (cases.length !== 260 || new Set(cases.map(sample => sample.id)).size !== 260 ||
...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, datasetHashes, model: "general/translation-llm", glossaryVersion: "v8", glossaryMode: "standard-conditional-native-html-semantic-markers",
    syntheticOnly: true, expected: 260 * rounds, rounds, apiCalls: apiCalls.length,
...
for (let round = 1; round <= rounds; round++) {
  for (const sample of cases) {
    ...
    if (results.length % 10 === 0 || serviceFailure) console.log(JSON.stringify({done: results.length,
      expected: 260 * rounds, completed: results.filter(row => row.completed).length, serviceFailure}));
