...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, model: "general/translation-llm", glossaryVersion: "v2", glossaryMode: "standard-plain-opaque-grouped-transport-decode",
...
for (let round = 1; round <= rounds; round++) {
  for (const sample of cases) {
    ...
    const translator = new TranslationLlmTranslator({projectId, location: "us-central1",
      glossaryZhEn: parent + "/glossaries/trade-zh-en-v2", glossaryEnZh: parent + "/glossaries/trade-en-zh-v2",
