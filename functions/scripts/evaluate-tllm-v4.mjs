...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, model: "general/translation-llm", glossaryVersion: "v4", glossaryMode: "standard-plain-semantic-markers-traditional-quotes",
...
for (let round = 1; round <= rounds; round++) {
  for (const sample of cases) {
    ...
    const translator = new TranslationLlmTranslator({projectId, location: "us-central1",
      glossaryZhEn: parent + "/glossaries/trade-zh-en-v4", glossaryEnZh: parent + "/glossaries/trade-en-zh-v4",
