...
const cases = [];
for (const file of ["synthetic-trade-cases.json", "synthetic-alternative-cases.json", "synthetic-tllm-holdout.json", "synthetic-tllm-v2-holdout.json", "synthetic-tllm-v4-holdout.json", "synthetic-tllm-v6-holdout.json"]) {
  ...
  cases.push(...data.filter(sample => sample.sourceLanguage !== "vi" && sample.targetLanguage !== "vi")
    .map(sample => ({...sample, set: file === "synthetic-tllm-v6-holdout.json" ? "holdout" : "regression"})));
}
if (cases.length !== 148 || new Set(cases.map(sample => sample.id)).size !== 148 ||
...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, model: "general/translation-llm", glossaryVersion: "v6", glossaryMode: "standard-plain-semantic-markers-traditional-quotes",
    syntheticOnly: true, expected: 148 * rounds, rounds, apiCalls: apiCalls.length,
...
for (let round = 1; round <= rounds; round++) {
  for (const sample of cases) {
    ...
    const translator = new TranslationLlmTranslator({projectId, location: "us-central1",
      glossaryZhEn: parent + "/glossaries/trade-zh-en-v6", glossaryEnZh: parent + "/glossaries/trade-en-zh-v6",
    ...
    if (results.length % 10 === 0 || serviceFailure) console.log(JSON.stringify({done: results.length,
      expected: 148 * rounds, completed: results.filter(row => row.completed).length, serviceFailure}));
