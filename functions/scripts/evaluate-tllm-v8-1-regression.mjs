...
const cases = [];
const datasetHashes = {};
for (const file of ["synthetic-trade-cases.json", "synthetic-alternative-cases.json", "synthetic-tllm-holdout.json", "synthetic-tllm-v2-holdout.json", "synthetic-tllm-v4-holdout.json", "synthetic-tllm-v6-holdout.json", "synthetic-tllm-v8-holdout.json"]) {
...
if (cases.length !== 180 || new Set(cases.map(sample => sample.id)).size !== 180 ||
...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, datasetHashes, model: "general/translation-llm", glossaryVersion: "v8", glossaryMode: "standard-plain-semantic-markers-traditional-quotes",
    syntheticOnly: true, expected: 180 * rounds, rounds, apiCalls: apiCalls.length,
...
for (let round = 1; round <= rounds; round++) {
  for (const sample of cases) {
    ...
    if (results.length % 10 === 0 || serviceFailure) console.log(JSON.stringify({done: results.length,
      expected: 180 * rounds, completed: results.filter(row => row.completed).length, serviceFailure}));
