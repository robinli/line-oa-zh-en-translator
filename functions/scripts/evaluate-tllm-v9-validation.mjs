...
for (const name of ["translation-llm-translator.js", "translation-llm-protection.js", "trade-policy.js", "translation-llm-subjects.js"]) {
...
const cases = [];
const datasetHashes = {};
for (const file of ["synthetic-trade-cases.json", "synthetic-alternative-cases.json", "synthetic-tllm-holdout.json", "synthetic-tllm-v2-holdout.json", "synthetic-tllm-v4-holdout.json", "synthetic-tllm-v6-holdout.json", "synthetic-tllm-v8-holdout.json", "synthetic-tllm-v9-holdout.json"]) {
  ...
  cases.push(...data.filter(sample => sample.sourceLanguage !== "vi" && sample.targetLanguage !== "vi")
    .map(sample => ({...sample, set: file === "synthetic-tllm-v9-holdout.json" ? "holdout" : "regression"})));
}
...
async function save() {
  ...
  await writeFile(output, JSON.stringify({startedAt, adapterVersion: TRANSLATION_LLM_ADAPTER_VERSION, implementationHashes, datasetHashes, model: "general/translation-llm", glossaryVersion: "v8", glossaryMode: "standard-plain-semantic-markers-traditional-quotes",
