...
test('all 35 historical paid entry points stop before credentials or provider calls',()=>{
 const inventory={files:readdirSync(new URL('./',import.meta.url)).filter(f=>/^(evaluate-|provision-)/.test(f)&&!f.includes('nmt'))};
 assert.equal(inventory.files.length,35);
