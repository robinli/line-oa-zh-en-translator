...
const root=new URL('../../',import.meta.url),dir=new URL('.local/tllm-context-quantity-verifier-v11/',root);mkdirSync(dir,{recursive:true});
const sha=x=>createHash('sha256').update(x).digest('hex'),read=p=>readFileSync(new URL(p,root)),json=p=>JSON.parse(read(p));
const review=json('functions/evaluation/tllm-context-quantity-repair-review.json');
if(process.argv.includes('--audit')){
 ...
 writeFileSync(new URL('regression-audit.json',dir),JSON.stringify(result,null,2).replace(/\n/g,'\r\n')+'\r\n');console.log(JSON.stringify(result,null,2));
