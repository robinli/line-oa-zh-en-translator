...
export function prepareLlmContext(text: string, names: readonly string[], ranges: ReadonlyArray<{start: number; length: number}> = [], targetLanguage?: string): PreparedLlmContext {
  ...
  // Resolve only the grammatical ellipsis with exactly one preceding quantified bag type.
  // Keep the original message and occurrences; no quantity, preference or condition is added.
  if (targetLanguage === "zh-TW") {
    const omitted = /\bIf\s+can['’]t\s+make\s+put\s+in\b/giu;
    prepared.text = prepared.text.replace(omitted, (whole: string, offset: number) => {
      const before = prepared.text.slice(0, offset);
      const preferred = [...before.matchAll(new RegExp("\\bIf you can make (" + prepared.prefix + "\\d+__) bags? is better[.]?", "giu"))];
      const weights = prepared.occurrences.filter(item => item.unitGroup && before.includes(item.token));
      if (preferred.length !== 1 || weights.length !== 1 || preferred[0]![1] !== weights[0]!.token || [...before.matchAll(/\bbags?\b/giu)].length !== 1 || /\bFIBC\b/iu.test(before)) return whole;
      return "If those bags cannot be made, use";
    });
  }
