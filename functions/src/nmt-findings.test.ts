...
it.each([false])("binds repeated native mentions to their quantity objects, retry=%s", async retry => {
  const source = "@Nora has confirmed 15 kg; @Nora has not confirmed 45 kg.";
  let call = 0;
  const output = (r: NmtGlossaryRequest) => ++call === 1 && retry ? "invalid" : div(atom(r, "@Nora", 0) + "已確認" + atom(r, "45 kg") + "；" + atom(r, "@Nora", 1) + "尚未確認" + atom(r, "15 kg") + "。");
...
it.each([false])("retains quantity paragraph ownership on both wire attempts %s", retry => {
  ...
  const request = {contents: [wire.encode()]} as NmtGlossaryRequest;
...
it("resolves only an explicit packaging fallback at the source and rejects invented registration", async () => {
  ...
  const output = (label: string) => (r: NmtGlossaryRequest) => '<div id="p0">Please keep the small bag plan.</div><div id="p1">The ' + label + ' is ' + atom(r, "560公斤") + ' FIBC bulk bags.</div>';
...
it.each(["請勿將貿易條件由 EXW 改為 CNF。", "不要把 EXW 改成 CNF。", "不要改 EXW 為 CNF。"])("accepts prohibited object-before-verb change: %s", async output => {
  await expect(translate("Do not change EXW into CNF.", r => div(output.replace("EXW", atom(r, "EXW")).replace("CNF", atom(r, "CNF"))))).resolves.toHaveProperty("calls", 1);
});
it.each(["請將 EXW 改為 CNF。", "無需把 EXW 改成 CNF。", "不是禁止將 EXW 改為 CNF。"])("rejects positive, optional or denied object-before-verb change: %s", async output => {
  await expect(translate("Do not change EXW into CNF.", r => div(output.replace("EXW", atom(r, "EXW")).replace("CNF", atom(r, "CNF"))))).rejects.toThrow("action_restriction_weakened");
});
