...
export function createLlmWireText(prepared: PreparedTradeText, maskNames = false) {
  // Stable, collision-free wire IDs make recorded requests and resumes byte-comparable.
  // Internal trade tokens stay random and never enter the external request.
  let index = createHash("sha256").update(prepared.original).digest().readUInt16BE(0) % 17576;
  let prefix = alphabetic(index);
  while (prepared.original.includes(prefix)) {index = (index + 1) % 17576; prefix = alphabetic(index);}
