...
const atom = (r: TranslationLlmRequest, value: string, occurrence = 0) => [...r.contents[0]!.matchAll(/<span\b[^>]*>([^<>]*)<\/span>/gu)].filter(m => m[1] === value)[occurrence]?.[0] ?? value;
const div = (body: string, id = 0) => '<div id="p' + id + '">' + body + '</div>';
async function translate(source: string, output: (r: TranslationLlmRequest) => string, retry = false, target = "zh-TW") {
  let calls = 0;
  const client: TranslationLlmClient = {async translateText(request) {calls++; return [{glossaryTranslations: [{translatedText: retry && calls === 1 ? "invalid" : output(request)}]}];}};
  return {text: await new TranslationLlmTranslator(options, client).translate(source, target === "en" ? "zh-TW" : "en", target), calls};
}
it.each([false, true])("accepts confirmed per-carton/per-bag natural pricing on attempt two=%s", async retry => {
...
it.each([false, true])("does not allow repeated equal amounts to exchange denominators, retry=%s", async retry => {
...
it.each([false, true])("copy-exact restores one original denominator after natural prefix, retry=%s", async retry => {
