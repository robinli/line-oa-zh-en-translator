...
export const TRANSLATION_LLM_ADAPTER_VERSION = "candidate-v12";
...
export class TranslationLlmTranslator implements Translator {
  ...
  public constructor(private readonly options: TranslationLlmOptions, private client?: TranslationLlmClient) {
    if (!/^[a-z][a-z0-9-]{4,62}$/u.test(options.projectId) ||
        !/^[a-z]+-[a-z]+\d$/u.test(options.location)) throw new Error("Invalid Translation LLM configuration.");
    ...
    for (const glossary of [options.glossaryZhEn, options.glossaryEnZh]) {
      if (!glossary.startsWith(this.parent + "/glossaries/") ||
          !/^[A-Za-z0-9_-]+$/u.test(glossary.slice((this.parent + "/glossaries/").length))) {
        throw new Error("Invalid Translation LLM glossary configuration.");
      }
    }
  }
  ...
  public async translateWithRanges(text: string, source: string, target: string, context?: TranslationContext) {
    if (!((source === "en" && target === "zh-TW") || (source === "zh-TW" && target === "en"))) {
    ...
    for (const range of [...context?.protectedRanges ?? []].sort((a, b) => a.start - b.start)) {
      if (!Number.isSafeInteger(range.start) || !Number.isSafeInteger(range.length) || range.start < end ||
          range.length <= 0 || range.start + range.length > text.length) {
        throw new TranslationQualityError("invalid_protected_range");
      }
      end = range.start + range.length;
    }
    const prepared = prepareTranslationLlmText(text, this.options.protectedNames ?? DEFAULT_PROTECTED_NAMES,
      context?.protectedRanges, target);
    const tokenPattern = new RegExp(prepared.prefix + "\\d+__", "gu");
    const paragraphs = prepared.text.split(/(\r\n|\r|\n)/u).map((part, index) => ({
      original: part, translatable: index % 2 === 0 && /\p{L}/u.test(part.replace(tokenPattern, "")),
    }));
    if (!paragraphs.some(part => part.translatable)) return restoreTradeTranslationWithRanges(prepared, prepared.text);
    // An auxiliary translation is used only when the full translation leaves a source quote unchanged.
    // The full paragraph always remains the primary translation input.
    const quotePattern = /[“「"]([^”」"\r\n]+)[”」"]/gu;
    const quotes = source === "en" ? [...new Set([...prepared.text.matchAll(quotePattern)]
      .map(match => match[1]!).filter(value => /[A-Za-z]/u.test(value.replace(tokenPattern, ""))))] : [];
    const paragraphCount = paragraphs.filter(part => part.translatable).length;
    for (let attempt = 1; attempt <= 2; attempt++) {
      const wire = createLlmWireText(prepared, attempt > 1);
      const html = prepared.rangeTokens.length ? createLlmHtmlWireText(prepared, wire) : undefined;
      const encodeRequest = html?.encode ?? wire.encode;
      const decodeResponse = html?.decodeTransport ?? decodeLlmTransport;
      const contents = [...paragraphs.filter(part => part.translatable).map(part => part.original), ...quotes].map(encodeRequest);
      ...
      try {
        ...
        [response] = await this.client.translateText({
          parent: this.parent, model: this.parent + "/models/general/translation-llm", contents,
          mimeType: html ? "text/html" : "text/plain", sourceLanguageCode: source, targetLanguageCode: target,
          glossaryConfig: {glossary: source === "en" ? this.options.glossaryEnZh : this.options.glossaryZhEn,
            ignoreCase: false, contextualTranslationEnabled: false},
        }, {timeout: 15000, retry: {retryCodes: []}});
      } catch {
        this.options.onMetric?.({engine: "translation-llm", direction: source + ":" + target, attempt,
          elapsedMs: Date.now() - started, inputCharacters, outputCharacters: 0, outcome: "service_error"});
        throw new TranslationLlmServiceError();
      }
      try {
        const translations = response?.glossaryTranslations;
        if (!Array.isArray(translations) || translations.length !== contents.length ||
            translations.some(part => !part || typeof part.translatedText !== "string" || !part.translatedText.trim())) {
        ...
        let index = 0;
        let masked = paragraphs.map(part => part.translatable ?
          decodeResponse(translations[index++]!.translatedText!) : wire.encode(part.original)).join("");
        if (quotes.length) {
          const replacements = new Map(quotes.map((quote, i) => [wire.encode(quote),
            translations[paragraphCount + i]!.translatedText!]));
          masked = masked.replace(quotePattern, (whole, inner: string) => {
            const translated = replacements.get(inner);
            return translated ? whole[0] + decodeResponse(translated) + whole.at(-1) : whole;
          });
        }
        masked = wire.decode(masked);
        if (target === "zh-TW") {
        ...
        const result = restoreTradeTranslationWithRanges(prepared, masked);
        validateLlmMentionSubjects(prepared, masked, source, target);
        const sourceBody = prepared.text.replace(tokenPattern, "");
        const targetBody = masked.replace(tokenPattern, "");
        if (/\p{L}/u.test(sourceBody) && (target === "zh-TW" ?
          !/\p{Script=Han}/u.test(targetBody) : (!/\p{Script=Latin}/u.test(targetBody) || /\p{Script=Han}/u.test(targetBody)))) {
          throw new TranslationQualityError("wrong_target_language");
        }
        validateTradeTerminology(text, result.text, target);
        validateLlmMeaning(text, result.text, target, prepared.protectedValues
          .filter(item => item.kind === "person" || item.kind === "person-mention").map(item => item.value),
          prepared.protectedValues.filter(item => item.kind === "quantity").map(item => item.value));
        // Insert only English word boundaries; keep source identity offsets in sync.
        if (target === "en" && result.ranges.length) {
          const original = result.text;
          let spaced = "", position = 0;
          for (const range of result.ranges) {
            const start = range.start;
            spaced += original.slice(position, start);
            if (/[\p{L}\p{N}]$/u.test(spaced)) spaced += " ";
            range.start = spaced.length;
            position = start + range.length;
            spaced += original.slice(start, position);
        ...
        this.options.onMetric?.({engine: "translation-llm", direction: source + ":" + target, attempt,
          elapsedMs: Date.now() - started, inputCharacters, outputCharacters, outcome: "success"});
        return result;
      } catch (error) {
        const failure = error instanceof TranslationQualityError ? error : new TranslationQualityError("invalid_response_format");
        const outputCharacters = Array.isArray(response?.glossaryTranslations) ?
          response.glossaryTranslations.reduce((sum, part) => sum +
            (typeof part?.translatedText === "string" ? [...part.translatedText].length : 0), 0) : 0;
        this.options.onMetric?.({engine: "translation-llm", direction: source + ":" + target, attempt,
          elapsedMs: Date.now() - started, inputCharacters, outputCharacters, outcome: "quality_rejected", reason: failure.reason});
